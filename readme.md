...
Select fields that use HoverCraft's normal visual field system should use the preferred custom downward SVG caret instead of inconsistent browser-native arrows. The native arrow should be disabled with `appearance: none`, `-webkit-appearance: none`, and `-moz-appearance: none`; the caret should use the default text color, sit at `right 14px center`, use `16px` background size, and reserve right-side space with `10px 44px 10px 16px` padding. This applies to normal selects, WooCommerce variation selects, WooCommerce quantity selects, and WooCommerce ordering selects unless a narrow plugin compatibility issue requires an exception.
