...
Header Center and Header Aside search forms should use the compact 392px sidebar/control width on desktop. Header Center may shrink if space is tight and aligns toward the branding area by default, while Header Aside search forms stay at the full width and other Header Aside widgets keep their natural content width. A future Customizer option should support Header Center alignment choices, such as left, center, or right, without requiring custom CSS.
