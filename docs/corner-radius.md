...
- `999px` only for intentionally circular elements, including small badges, counters, labels, sale chips, status chips, tiny inline metadata pills, author avatars, byline avatars, comment avatars, other intentionally circular identity photos, range slider thumbs, and small icon-only controls.
...
Do not use arbitrary one-off radius values. If a new component looks like a button or form control, use `4px`. If it looks like a visible card, widget, panel, or island, use `8px`. If it is an image, table, structural edge, plain article-flow section, or document-style content area, use `0px`.
