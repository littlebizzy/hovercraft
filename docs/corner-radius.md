...
Use `4px` for buttons, calls to action, form fields, selects, textareas, search fields, quantity controls, add-to-cart controls, checkout controls, comment forms, notices, alerts, messages, pagination links, tabs, account navigation items, bbPress forms, bbPress actions, compact controls, inline `code` snippets, and small overlay captions.
...
If a new component looks like a button, form control, compact control, or inline code snippet, use `4px`.

If it looks like a visible card, widget, panel, island, or block-level code container, use `8px`.

If it is an image, table, structural edge, plain article-flow section, document-style content area, or nested code element inside a block-level code container, use `0px`.
